...
const NavLinks = ({ isHidden, handleClick }) => {
  ...
  return (
    <div className={`${hiddenClass} px-[7.5vw] py-4 fixed w-screen h-screen bg-secondary top-0 left-0`}>
...
const NavBar = (props) => {
  ...
  return (
    <nav className="flex justify-between py-4 px-[10vw]">
      <button><MenuIcon onClick={handleClick} className="text-3xl" /></button>
      <h1 className="text-2xl">
        <Link to="/">Blvck</Link>
      </h1>

      <NavLinks isHidden={isHidden} quantity={quantity} handleClick={handleClick}/>
      <Link to="/cart"><BagIcon className="text-3xl"/> {quantity}</Link>
    </nav>
  );
};
